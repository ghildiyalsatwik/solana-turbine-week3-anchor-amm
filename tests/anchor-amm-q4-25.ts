...
import { ASSOCIATED_TOKEN_PROGRAM_ID, createAssociatedTokenAccountInstruction,
  createMint, getAssociatedTokenAddressSync, mintTo, TOKEN_PROGRAM_ID } from "@solana/spl-token";
...
describe("anchor-amm-q4-25", () => {
  ...
  before(async () => {
    ...
    mintX = await createMint(provider.connection, provider.wallet.payer, depositor.publicKey, null, 6);

    mintY = await createMint(provider.connection, provider.wallet.payer, depositor.publicKey, null, 6);
